...
que conterá a quantidade e o valor total de prestações pagas no dia. O cálculo
do valor a ser pago é feito da seguinte forma. Para pagamentos sem atraso,
cobrar o valor da prestação. Quando houver atraso, cobrar 3% de multa, mais
...
def valorPagamento():
    ...
    while valor != 0:
        ...
        if valor <= 0:
            ...
            nPagamento += 1
        diaAtraso = int(input('Digite o número de dias de atraso: '))
        if diaAtraso > 0:
            multa = valor*0.03
            juros = (valor+multa)*(diaAtraso*0.001)
            total = valor+multa+juros
            saldo += total
            print(f'Valor da prestação R$ {total:.2f} reais')
        else:
            print(f'Valor da prestação R$ {valor} reais')
    print(f'N° pagamentos: {nPagamento}')
    print(f'Valor total: R$ {saldo:.2f} reais')
